...
#define POLL_TX_TO_RESP_RX_DLY_UUS 140*5
/* Receive response timeout. See NOTE 5 below. */
#define RESP_RX_TIMEOUT_UUS 210*5
...
char dist_str[16] = {0};
extern void sleep_ms(unsigned int time_ms);
        ...
        if (status_reg & SYS_STATUS_RXFCG)
        {
            ...
            * As the sequence number field of the frame is not relevant, it is cleared to simplify the validation of the frame. */
            ...
            if (memcmp(rx_buffer, rx_resp_msg, ALL_MSG_COMMON_LEN) == 0)
            {
                ...
                rt_sprintf(dist_str, "%d DIST: %3.2f m", frame_seq_nb, distance);
                lcd_display_str(dist_str);

                extern HeartBeat heartbeat;
                heartbeat.distance[frame_seq_nb] = (int)(distance*100);
